...
class HeapPriorityQueue(PriorityQueueBase): # base class defines Item
    ...
    def upheap(self, j):
        ...
        if j > 0 and self.data[j] < self.data[parent]:
            self.swap(j, parent)
            self.upheap(parent)  # recur at the position of the parent
    ...
    def downheap(self, j):
        if self._has_left(j):
            ...
            if self.data[small_child] < self.data[j]:
                ...
                self.downheap(small_child)  # recur at the position of the small child
    ...
    def remove_min(self):
        ...
        self.downheap(0)  # then fix the new root
        return (item._key, item._value)
